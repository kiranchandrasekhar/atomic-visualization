...
export class ElectronAnimation extends CanvasAnimation {
  ...
  /*  Electron Rendering */
  private electronGeometry: Electron;
  private electronRenderPass: RenderPass;

  private electronPositions: Float32Array[];
  private electronCount: number;
  private electronsLoaded: boolean;
  private electronRadius: number;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.gui = new GUI(this.canvas2d, document.getElementById("controls-container") as HTMLDivElement, this);
   ...
   */
  public setElectronPositions(n:number, l:number, m:number, config?:number[]):void {
    this.electronsLoaded = false;
    if(config == undefined) {
      //Load in just one orbital from the specified N, L, M (for hydrogen)
      this.getElectronPositions(n, l, m).then((data:number[]) => {
      ...
      for(let i = 0; i < config.length; i += 3) {
        ...
        this.getElectronPositions(config[i], config[i+1], config[i+2]).then((data:number[]) => {
          //Don't necessarily want to sample all the points, in order to speed up rendering with many orbitals
          let sampledPos:Float32Array = new Float32Array(data.length / totalCount);
   ...
   *  specified as [n_i, l_i, m_i, n_{i+1}, m_{i+1}, l_{i+1}, ...]
   */
  public async getElectronPositions(n:number, l:number, m:number): Promise<number[]> {
  ...
  private initElectron(): void {
    ...
    this.electronRenderPass.addAttribute("aVertPos",
      3,
      this.ctx.FLOAT,
      false,
      ...
      0,
      undefined,
      this.electronGeometry.positionsFlat()
    );
    
    this.electronRenderPass.addAttribute("aNorm",
      3,
      this.ctx.FLOAT,
      false,
      3 * Float32Array.BYTES_PER_ELEMENT,
      0,
      undefined,
      this.electronGeometry.normalsFlat()
    );

    this.electronRenderPass.addInstancedAttribute("aOffset",
      4,
      this.ctx.FLOAT,
      false,
    ...
    this.electronRenderPass.addUniform("uLightPos",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform3fv(loc, this.lightPosition.xyz);
    });
    this.electronRenderPass.addUniform("uProj",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all()));
    });
    this.electronRenderPass.addUniform("uView",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all()));
    });
    this.electronRenderPass.addUniform("uTime",
    (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
      gl.uniform1f(loc, this.getTime());
    });
    this.electronRenderPass.addUniform("uRadius",
    (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
      gl.uniform1f(loc, this.getElectronRadius());
    });
    this.electronRenderPass.addUniform("uNLM",
    (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
      gl.uniform3fv(loc, this.getQuantumNumbers());
    });
    this.electronRenderPass.addUniform("uCutaway",
    (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
      gl.uniform1f(loc, this.getCutaway());
    });
    this.electronRenderPass.addUniform("uShading",
    (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
      gl.uniform1f(loc, this.getShading());
    });
    this.electronRenderPass.addUniform("uGradient",
    (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
  ...
  public getTime():number {
    return ((Date.now() - this.initialTime)/10) % 48_000;
  }

  public getElectronRadius():number {
    return this.electronRadius;
  }

  public getQuantumNumbers():Float32Array {
    return new Float32Array([this.nlm.x, this.nlm.y, this.nlm.z]);
  }

  public getCurrentGradient():number {
  ...
  public updateFromGUI():void {
    ...
    if(this.atomicNumber == 1) {
      let n:number = this.gui.get("n-number");
      let l:number = this.gui.get("l-number");
      let m:number = this.gui.get("m-number");
...
export function initializeCanvas(): void {
  ...
  /* Start drawing */
  const canvasAnimation: ElectronAnimation = new ElectronAnimation(canvas);
